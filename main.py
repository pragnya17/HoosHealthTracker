...
def main():
    if 'user_id' in session and session['user_id']:
        ...
        return render_template("main.html", emotionResult=emotionResult, exerciseResult=exerciseResult,
                               sleepResult=sleepResult, foodResult=foodResult)
    else:
        return redirect(url_for('login'))
...
def profile():
    if 'user_id' in session and session['user_id']:
        ...
        return render_template("profile.html", first_name=first_name, last_name=last_name, height=height,
...
def nutrition_info(food_id):
    ...
    if food['household_serving_fulltext'] is not None:
        if str(int(food['serving_size'])) in food['household_serving_fulltext'] or str(food['serving_size']) in food[
